...
class make_plots():
    ...
    def make_plot(self, X, Y, info, x_title, baseline_x, baseline_y, filename):
        ...
        for x,y,i in zip(X, Y, info):
                ...
                if num_layers == 0:
                        if labled[0]:
                                self.sc.append(self.ax.scatter(x,y, color='lightsteelblue'))
                        else:
                                self.sc.append(self.ax.scatter(x,y, color='lightsteelblue', label='0 CNN layer'))
                                labled[0] = True
                elif num_layers == 1:
                        if labled[1]:
                                self.sc.append(self.ax.scatter(x,y, color='cornflowerblue'))
                        else:
                                self.sc.append(self.ax.scatter(x,y, color='cornflowerblue', label='1 CNN layer'))
                                labled[1] = True
                elif num_layers == 2:
                        if labled[2]:
                                self.sc.append(self.ax.scatter(x,y, color='royalblue'))
                        else:
                                self.sc.append(self.ax.scatter(x,y, color='royalblue', label='2 CNN layer'))
                                labled[2] = True
                else:
                        if labled[3]:
                                self.sc.append(self.ax.scatter(x,y, color='midnightblue'))
                        else:
                                self.sc.append(self.ax.scatter(x,y, color='midnightblue', label='3 CNN layer'))
                                labled[3] = True
